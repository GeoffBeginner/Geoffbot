  ...
  return new Promise((resolve, reject) => {
    login({
      ...
      try {
        ...
        if (!userInfo || !userInfo[userid]?.name || !userInfo[userid]?.profileUrl || !userInfo[userid]?.thumbSrc) throw new Error('Unable to locate the account; it appears to be on bad position like suspended or locked state.');
        const {
          name,
          profileUrl,
          thumbSrc
          ...
          if (event.body && event.body?.toLowerCase().startsWith(prefix.toLowerCase()) && aliases(command)?.name) {
            if (blacklist.includes(event.senderID)) {
              ...
              return;
					...
					api.changeNickname(`》 ${prefix} 《 ✞ ➠ 𝔾ℕ𝕀𝕄𝕆𝕌𝕊-𝔹𝕆𝕋`, threadID, userid);
					...
					.then(response => {
					fs.writeFileSync(gifPath, response.data);					  return api.sendMessage(`🔴🟢🟡\n\n✓ 𝔾𝕣𝕠𝕦𝕡-ℂ𝕠𝕟𝕟𝕖𝕔𝕥𝕚𝕠𝕟-𝕊𝕌ℂℂ𝔼𝕊𝕊! \n➭ Bot Prefix: ${prefix}\n➭ Admin: Geoff(j)\n➭ Facebook: ‹https://www.facebook.com/profile.php?id=61558748810480›\n➭ Use ${prefix} help to view commands details\n➭ Added bot at: ⟨ ${time} ⟩〈 ${thu} 〉`, event.threadID,
					);
					})
					.catch(error => {
					 ...
					 if (event.logMessageType === "log:unsubscribe") {
						 api.getThreadInfo(event.threadID).then(({ participantIDs }) => {
							 ...
							 api.getUserInfo(leaverID, (err, userInfo) => {
								 ...
								 const type = (event.author == event.logMessageData.leftParticipantFbId) ? "𝕝𝕖𝕗𝕥 𝕥𝕙𝕖 𝕘𝕣𝕠𝕦𝕡." : "𝕜𝕚𝕔𝕜𝕖𝕕 𝕓𝕪 𝔸𝕕𝕞𝕚𝕟 𝕠𝕗 𝕥𝕙𝕖 𝕘𝕣𝕠𝕦𝕡"; api.sendMessage(`${name} has ${type} the group.`, event.threadID);
							 });
						 })
					 }
					}
									...
									const fs = require('fs');
																		...
																		while ((match = gdriveLinkPattern.exec(event.body)) !== null) {
																			...
																			try {
																				...
																				await api.sendMessage({ body: `𝖠𝗎𝗍𝗈 𝖽𝗈𝗐𝗇 𝖦𝗈𝗈𝗀𝗅𝖾 𝖣𝗋𝗂𝗏𝖾 𝖫𝗂𝗇𝗄 \n\n𝙵𝙸𝙻𝙴𝙽𝙰𝙼𝙴: ${fileName}\n\n𝔾ℕ𝕀𝕄𝕆𝕌𝕊-𝔹𝕆𝕋 𝗕𝗢𝗧 𝟭.𝟬.𝟬𝘃`, attachment: fs.createReadStream(destPath) }, event.threadID);
														...
														if (event.body !== null) {
															...
															const downloadAndSendFBContent = async (url) => {
																try {
																	...
																	return api.sendMessage({ body: "𝖠𝗎𝗍𝗈 𝖣𝗈𝗐𝗇 𝖥𝖺𝖼𝖾𝖻𝗈𝗈𝗄 𝖵𝗂𝖽𝖾𝗈\n\n 𝔾ℕ𝕀𝕄𝕆𝕌𝕊-𝔹𝕆𝕋 𝟭.𝟬.𝟬𝘃", attachment: fs.createReadStream(fbvid) }, event.threadID, () => fs.unlinkSync(fbvid));
																}
																catch (e) {
